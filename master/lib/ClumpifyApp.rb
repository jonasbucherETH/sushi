...
Refer to <a href='https://jgi.doe.gov/data-and-tools/software-tools/bbtools/bb-tools-user-guide/clumpify-guide/'>https://jgi.doe.gov/data-and-tools/software-tools/bbtools/bb-tools-user-guide/clumpify-guide/</a>
  ...
  def commands
    ...
    if @params['paired']
      ...
      output_R2 = File.basename(@dataset['Read2']).gsub('_val_2.fq.gz', '.fq.gz')
      command << " out=#{output_R1}"
      command << " in2=#{File.join(SushiFabric::GSTORE_DIR, @dataset['Read2'])}"
      command << " out2=#{output_R2}"
    else
      output_R1 = File.join(@result_dir, "#{File.basename(@dataset['Read1']).gsub('_trimmed.fq.gz', '.fq.gz')}")
      command << " out=#{output_R1}"
    end
    # [options]
